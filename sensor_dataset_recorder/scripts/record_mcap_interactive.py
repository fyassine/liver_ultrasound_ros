#!/usr/bin/env python3
"""
Interactive MCAP recorder - waits for user input before starting.
Use with record_inside_rviz.launch to see visualization first.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Interactive MCAP recorder')
    ...
    rospy.init_node('mcap_recorder_interactive', anonymous=True)
    
    recording_fps = rospy.get_param('~recording_fps', 30.0)
    duration = rospy.get_param('~duration', args.duration)
    wait_for_input = rospy.get_param('~wait_for_input', True)
    ...
    if wait_for_input:
        print("\n" + "="*50)
        print("  MCAP RECORDER - Ready to record")
        print("="*50)
