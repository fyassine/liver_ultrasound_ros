...
class SurfaceFollower:
    ...
    def follow_surface(self):
        ...
        # Get current joint values - robot spawns near home already
        current_joints = self.move_group.get_current_joint_values()
        rospy.loginfo(f"Current joint values at start: {current_joints}")
        
        # Skip explicit home movement since robot spawns at home position in Gazebo
        # The spawn command in liver_simulation.launch sets all joints to 0.0
        rospy.loginfo("Skipping home movement - robot spawned at home position.")
    ...
    def _apply_contact_push(self, touch_pose):
        ...
        # Wait a bit for contact to stabilize
        rospy.sleep(0.5)
        
        rospy.loginfo("Contact push complete.")
        
        # Restore default scaling
        self.move_group.set_max_velocity_scaling_factor(1.0)
        self.move_group.set_max_acceleration_scaling_factor(1.0)
        
        log_success("Surface following sequence complete.")
